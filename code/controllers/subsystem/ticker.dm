...
/datum/controller/subsystem/ticker/proc/setup()
	...
	if(config.jobs_have_department_access && SSticker.mode.num_players() >= config.department_access_trigger)
		config.jobs_have_department_access = FALSE

	// create player characters and transfer clients
	// TODO: refactor these start_watch() and stop_watch() calls to tracing
	var/watch = start_watch()
	create_characters()
	log_debug("Creating characters took [stop_watch(watch)]s")

	// put mobs in their spawn locations
	watch = start_watch()
	populate_spawn_points()
	log_debug("Populating spawn points took [stop_watch(watch)]s")

	// gather everyone's minds
	watch = start_watch()
	//collect_minds()
	for(var/mob/living/player in GLOB.player_list)
		if(player.mind)
			minds += player.mind
	log_debug("Collecting minds took [stop_watch(watch)]s")

	// apply outfits and loadouts to the characters
	watch = start_watch()
	equip_characters()
	...
	// tell everybody who is playing on the station
	watch = start_watch()
	announce_manifest_to_discord()
	log_debug("Announcing manifest to Discord took [stop_watch(watch)]s")
