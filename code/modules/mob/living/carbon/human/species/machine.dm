...
/datum/action/innate/change_monitor/Activate()
	...
	if(!robohead.is_monitor) //If they've got a prosthetic head and it isn't a monitor, they've no screen to adjust. Instead, let them change the colour of their optics!
		...
		for(var/i in GLOB.hair_styles_public_list)
			...
			if((head_organ.dna.species.name in tmp_hair.species_allowed) && (robohead.model_name in tmp_hair.models_allowed)) //Populate the list of available monitor styles only with styles that the monitor-head is allowed to use.
				hair += i
