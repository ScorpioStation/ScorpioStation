	...
	if(useMS)
		if(prob(5))
			...
			switch(pick(1,2,3,4,5,6,7))
				...
				if(6)
					sender = pick("Ark Soft Morale Divison","Feeling Lonely?","Bored?","www.wetskrell.nt")
					...
					"Wetskrell.nt only provides the higest quality of male entertaiment to Ark Soft Employees.",\
