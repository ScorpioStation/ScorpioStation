	...
	if(!newMsg.body)
		...
		switch(event_type)
			if(RESEARCH_BREAKTHROUGH)
				...
				Ark Soft declined to comment as to whether this could impinge on profits."
			...
			if(ELECTION)
				...
				"I will maintain Ark Soft's record profits","I believe in our future","We must return to our moral core","Just like... chill out dudes")]'."

			if(RESIGNATION)
				newMsg.body = "Ark Soft regretfully announces the resignation of [pick("Sector Admiral","Division Admiral","Ship Admiral","Vice Admiral")] [random_name(pick(MALE,FEMALE))]."
				if(prob(25))
