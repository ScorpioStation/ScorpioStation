...
/client/proc/karmashopmenu()
	...
	switch(karma_tab)
		if(0) // Job Unlocks
			...
			if(!("Ark Soft Representative" in joblist))
				dat += "<a href='?src=[UID()];karmashop=shop;KarmaBuy=3'>Unlock Ark Soft Representative -- 30KP</a><br>"
			else
				dat += "Ark Soft Representative - <font color='green'>Unlocked</font><br>"
