 ...
 *		e.g aa = /turf/closed/staticwall{icon_state = "rock"}
 ...
 * Variable model is one full map line (e.g /turf/closed/staticwall{icon_state = "rock"},/area/mine/dangerous/explored)
 *
 * WORKING :
 *
...
/datum/dmm_suite/proc/parse_grid(model = "", xcrd = 0, ycrd = 0, zcrd = 0, datum/dmm_suite/loaded_map/LM)
	...
	*/
	...
	var/list/members // will contain all members (paths) in model (in our example : /turf/closed/staticwall and /area/mine/dangerous/explored)
	...
	if(cached)
		...
		do
			// finding next member (e.g /turf/closed/staticwall{icon_state = "rock"} or /area/mine/dangerous/explored)
