...
/proc/spawn_rivers(target_z, nodes = 4, turf_type = /turf/open/ind_floor/plating/lava/smooth/lava_land_surface, whitelist_area = /area/lavaland/surface/outdoors, min_x = RANDOM_LOWER_X, min_y = RANDOM_LOWER_Y, max_x = RANDOM_UPPER_X, max_y = RANDOM_UPPER_Y)
...
/turf/proc/Spread(probability = 30, prob_loss = 25, whitelisted_area)
	...
	for(var/F in RANGE_TURFS(1, src) - src)
		...
		if(!T || (T.density && !ismineralturf(T)) || isindesctructibleturf(T) || (whitelisted_area && !istype(new_area, whitelisted_area)) || (T.flags & NO_LAVA_GEN) )
			continue
