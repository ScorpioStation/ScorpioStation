...
/turf/open/floor/mineral/plasma/welder_act(mob/user, obj/item/I)
...
/turf/open/floor/mineral/gold/fancy
...
/turf/open/floor/mineral/silver/fancy
...
/turf/open/floor/mineral/bananium/Entered(mob/living/M)
	.=..()
	if(!.)
		if(istype(M))
			squeek()
...
/turf/open/floor/mineral/bananium/lubed/Initialize(mapload)
	. = ..()
	MakeSlippery(TURF_WET_LUBE, INFINITY)

/turf/open/floor/mineral/bananium/lubed/pry_tile(obj/item/C, mob/user, silent = FALSE) //I want to get off Mr Honk's Wild Ride
	if(ishuman(user))
