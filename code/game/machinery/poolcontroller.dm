...
/obj/machinery/poolcontroller/Initialize(mapload)
	...
	for(var/turf/T in contents_loop)
		if(istype(T, /turf/open/floor/beach/water))
			...
			linkedturfs += T
		else if(istype(T, /turf/open/beach/water))
			var/turf/open/beach/water/W = T
			W.linkedcontroller = src
			linkedturfs += T
...
/obj/machinery/poolcontroller/proc/processMob()
	for(var/M in mobinpool) //They're already typecasted when entering the turf
		...
		if(!istype(get_turf(M), /turf/open/floor/beach/water) && !istype(get_turf(M), /turf/open/beach/water)) // Water component when?
			mobinpool -= M
			continue
