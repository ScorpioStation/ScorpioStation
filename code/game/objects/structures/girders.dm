		...
		if(istype(S, /obj/item/stack/sheet/wood))
			if(state == GIRDER_DISPLACED)
				...
				if(do_after(user, 40 * W.toolspeed, target = src))
					...
					Tsrc.ChangeTurf(/turf/closed/wall/mineral/wood)
					for(var/turf/closed/wall/mineral/wood/X in Tsrc.loc)
						if(X)
			...
			if(state == GIRDER_DISPLACED)
				...
				if(state == GIRDER_REINF)
					...
					if(do_after(user, 50, target = src))
						...
						var/turf/Tsrc = get_turf(src)
						Tsrc.ChangeTurf(/turf/closed/wall/r_wall)
						for(var/turf/closed/wall/r_wall/X in Tsrc.loc)
							if(X)
								X.add_hiddenprint(usr)
						qdel(src)
					return
